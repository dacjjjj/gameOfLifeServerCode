...
func (g *GolMasterRunner) MasterStart(initReq stubs.InitialRequest, finalRes *stubs.FinalResponse) (err error) {
	...
	blankWorld := make([][]byte, sizeOfWorld)
	...
	if len(responses) < 4 {
		log.Printf("Warning: Expected %d responses, but received %d. Some workers did not respond.\n", 4, len(responses))
	}

	// Recombine the responses into blankWorld
	for i := 0; i < len(responses); i++ {
		receivedWorld := responses[i].FinalWorld
		// Calculate the vertical range for this worker’s section
		startRow := i * sizeOfWorld / 4
		endRow := (i + 1) * sizeOfWorld / 4

		for y := startRow; y < endRow; y++ {
			copy(blankWorld[y], receivedWorld[y-startRow]) // Copy each row slice directly
		}
	}

	FinalResponseWorld := blankWorld
	finalRes.FinalWorld = FinalResponseWorld
	log.Printf("passingBack from server.\n", FinalResponseWorld, initReq.Turns)
