...
func distributor(p Params, c distributorChannels) {
	...
	var ticker *time.Ticker
	ticker = time.NewTicker(2 * time.Second)
	go background(client, done, ticker, c)
...
func background(client *rpc.Client, done chan bool, ticker *time.Ticker, c distributorChannels) {
	...
	for {
		...
		case <-ticker.C:
			...
			err := client.Call(stubs.TickTime, request, response)
			if err != nil {
				log.Fatalf("Error during RPC call: %v", err)
			}
